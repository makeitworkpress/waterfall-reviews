...
class Ajax extends Base
{
    ...
    public function get_chart_data()
    {
        ...
        $args = [
            ...
            "include" => array_filter(
                explode(",", sanitize_text_field($_POST["include"])),
                function ($v) {
                    return is_numeric($v);
                },
            ),
    ...
    public function load_tables()
    {
        ...
        $args = [
            "attributes" => isset($_POST["attributes"])
                ? array_filter(
                    explode(",", sanitize_text_field($_POST["attributes"])),
                )
                : [],
            ...
            "groups" => isset($_POST["groups"])
                ? array_filter(
                    explode(",", sanitize_text_field($_POST["groups"])),
                )
                : [],
            "load" => true,
            "price" => false,
            "properties" => isset($_POST["properties"])
                ? array_filter(
                    explode(",", sanitize_text_field($_POST["properties"])),
                )
                : [],
            "reviews" => array_filter($_POST["reviews"], function ($v) {
                return is_numeric($v);
            }),
            ...
            "view" => isset($_POST["view"])
                ? sanitize_text_field($_POST["view"])
                : "tabs",
    ...
    public function filter_reviews()
    {
        ...
        $args = [
            ...
            "fields" => "ids",
            "post_status" => "publish",
            "posts_per_page" => isset($_POST["number"])
                ? intval($_POST["number"])
        ...
        if (isset($_POST["price_min"]) && $_POST["price_min"]) {
            ...
            $args["meta_query"]["prices"]["value"] = intval(
                $_POST["price_min"],
            );
        }
        ...
        if (
            ...
            $args["meta_query"]["prices"]["key"] = "price";
            $args["meta_query"]["prices"]["type"] = "NUMERIC";
            $args["meta_query"]["prices"]["value"] = [
                intval($_POST["price_min"]),
                intval($_POST["price_max"]),
            ];
        }
        ...
        if (isset($_POST["rating"]) && $_POST["rating"]) {
            $options = wf_get_data();
            $compare =
                $options["rating_maximum"] == $_POST["rating"] ? "=" : ">=";
        ...
        // Taxonomy terms or category archive
        ...
        $reviews = new Views\Components\Reviews([
            "attributes" => ["class" => "archive-posts wfr-reviews-component"],
            "query_args" => $args,
        ]);

        // Send our filtered results
        wp_send_json_success([
            "html" => $reviews->render(false),
            "posts" => $posts,
        ]);
    }
}
